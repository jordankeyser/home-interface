...
// Alpha Vantage free tier is rate-limited; refresh one symbol at a time.
...
const StocksModule = () => {
  ...
  // Fetch quotes gradually (one symbol per tick) to play nicely with Alpha Vantage limits.
  useEffect(() => {
    let cancelled = false;

    // reset cycle when the symbol list changes
    ...
    async function refreshOne() {
      ...
      try {
        if (cancelled) return;
        const idx = fetchIndexRef.current % symbols.length;
        const sym = symbols[idx];
        fetchIndexRef.current = idx + 1;
    ...
    refreshOne();
    const id = setInterval(refreshOne, SYMBOL_REFRESH_MS);
    return () => {
      cancelled = true;
      clearInterval(id);
    };
